...
def main():
    ...
    imagenet_mean = 117.0
    t_preprocessed = tf.compat.v1.expand_dims(t_input-imagenet_mean, 0)
    ...
    def render_naive(t_obj, img0=img_noise, iter_n=20, step=1.0):
        t_score = tf.compat.v1.reduce_mean(t_obj) # defining the optimization objective
        t_grad = tf.compat.v1.gradients(t_score, t_input)[0] # behold the power of automatic differentiation!
    ...
    def resize(img, size):
        img = tf.compat.v1.expand_dims(img, 0)
    ...
    def render_deepdream(t_obj, img0=img_noise,
            ...
            t_grad = tf.compat.v1.gradients(t_score, t_input)[0]
        ...
        #Pick Layer to Enchance
    layer = "mixed4d_3x3_bottleneck_pre_relu"
    channel = 139

    img0 = PIL.Image.open("doge.jpg")
    img0 = np.float32(img0)

    #Apply Gradient Descent to Layer
    render_deepdream(tf.compat.v1.square(T('mixed4c')), img0)
